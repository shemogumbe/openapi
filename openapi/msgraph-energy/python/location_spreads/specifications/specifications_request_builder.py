...
class SpecificationsRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /locationSpreads/specifications
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, Dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/locationSpreads/specifications{?filter%5BcurrencyId%5D*,filter%5BmarketDeliveryProfileName%5D*,filter%5BmarketId%5D*,filter%5BunitId%5D*,page%5Bafter%5D*,page%5Bsize%5D*}", path_parameters)
    
    def by_id(self,id: str) -> SpecificationsItemRequestBuilder:
        """
        Gets an item from the GraphPythonv1.locationSpreads.specifications.item collection
        param id: The Id of the location spread specification for which the user wants to retrieve data.
    ...
    async def get(self,request_configuration: Optional[RequestConfiguration[SpecificationsRequestBuilderGetQueryParameters]] = None) -> Optional[SpecificationsGetResponse]:
        """
        Returns a list of location spread specifications sorted by `name` in ascending order. If there is no data, then an empty array will be returned.
